...
const Dashboard = () => {
  ...
  const sortElement = useSelector(
    (state: RootState) => state.tickets.sortedElement
  );
  ...
  useEffect(() => {
    setData(
      tickets.filter((e) => {
        if (filteredCompany.length && !filteredCompany.includes(e.company)) {
          return false;
        }
  ...
  const newTickets = data.map((ticket) => (
    <div
      className='flex flex-col p-10 pb-9 bg-[#E8EBF2] rounded-[10px] w-full'
      key={ticket.id}
    >
      ...
      <div className='flex justify-between mt-6'>
        <div>
          <p className='font-mediium text-base text-[#858AE3]'>
            {ticket.from} - {ticket.to}
          </p>
          <p className='font-medium text-base pt-1 text-purple-900'>
            {ticket.time.departure} - {ticket.time.arrival}
          </p>
        </div>
        <div>
          <p className='font-mediium text-base text-[#858AE3]'>В пути</p>
          <p className='font-medium text-base pt-1 text-purple-900'>
            {ticket.duration} часа{' '}
          </p>
        </div>
        <div>
  ...
  return (
    <div>
      ...
      <button
        ...
        className='font-bold rounded-[10px] text-2xl mt-[74px] flex justify-center py-4 text-center w-full mb-[60px] bg-purple-900 text-[#F7F9F7]'
      >
